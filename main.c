...
volatile unsigned int blink_sequence[] = {1, 1, 1, 1, 1, 3, 3, 1, 3, 1, 3, 3, 1, 1, 1, 1, 1, 7};	// Values that will be multiplied by blink_interval
volatile unsigned int i; // index into blink_sequence
...
int main(void) {
	  ...
	  i = 0;							// initialize the index into blink_sequence
...
interrupt void WDT_interval_handler(){
  if (--blink_counter==0){          // decrement the counter and act only if it has reached 0
	  ...
	  blink_counter = blink_sequence[i]*blink_interval; // reset blink_counter
